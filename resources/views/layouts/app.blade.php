...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    if(closeBtn) closeBtn.addEventListener("click",()=>{popup.classList.remove("active");setTimeout(()=>popup.style.display="none",250);});
    if(popup) popup.addEventListener("click",e=>{if(e.target===popup){popup.classList.remove("active");setTimeout(()=>popup.style.display="none",250);}});
    if(addBtn) addBtn.addEventListener("click",()=>{const div=document.createElement("div");div.classList.add("d-flex","gap-2","mb-2","anggota-row");div.innerHTML=`<input type="text" name="anggota[]" class="form-control" placeholder="Masukkan nama anggota"><button type="button" class="btn btn-danger btn-sm remove-anggota">Hapus</button>`;anggotaContainer.appendChild(div);});
    ...
    async function loadNotifications(){
        try{
            ...
            notifList.innerHTML='';
            if(data.length===0){notifList.innerHTML=`<div class="text-center text-secondary py-3">Tidak ada notifikasi.</div>`; return;}
            data.forEach(notif=>{
                const item=document.createElement("div");
                item.classList.add("notif-item");if(notif.read)item.classList.add("read");
                item.innerHTML=`<div class="notif-title">${notif.title}</div><div class="notif-time">${new Date(notif.created_at).toLocaleString()}</div>`;
